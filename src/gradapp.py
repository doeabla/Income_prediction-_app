...
def predict_income_limit(age, gender, education, work_class, marital_status, race, is_hispanic,
                         employment_commitment, wage_per_hour, working_week_per_year, industry_code_main,
                         occupation_code_main, tax_status, citizenship, pipeline=loaded_pipeline,
                         encoder=loaded_encoder):
    try:
        input_data = pd.DataFrame(
            [[age, gender, education, work_class, marital_status, race, is_hispanic,
              employment_commitment, wage_per_hour, working_week_per_year, industry_code_main,
        ...
        # Convert the predicted income level to a string
        return str(decoded_income_level[0])
...
EmpCommit = gr.Dropdown(label="Employment Commitment", choices=["Full-time schedules", "Not in labor force", "PT for non-econ reasons usually FT", "PT for econ reasons usually PT", "Unemployed full-time", "PT for econ reasons usually FT", "Unemployed part- time", "PT for non-econ reasons usually PT", "PT for econ reasons usually FT", "Unemployed part-time"])
...
gr.Interface(
  ...
  outputs=gr.Label(),
   title="<b>Income Prediction App</b>",
   description=f"<div style='text-align: center;'><img src='{image_path}' style='width: 50%;'><br><br>{designers_names}</div><br><b>Enter Employee information to predict Income limit.</b>",
   live=True
